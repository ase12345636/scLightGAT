...
def preprocess_adata(adata, upper_lim=98, lower_lim=2, use_hvg=False, doublet_detect=True, keep_doublets=True, expected_doublet_rate=0.06):
    ...
    logger.info("Filtering RP & MT genes and calculating QC metrics")
...
def visualization_process(adata, res=0.8, batch_key=None, min_cluster_size=50):
    logger.info("Starting visualization process")
    
    logger.info("Performing PCA (Default components are 50)")
    ...
    if len(small_clusters) > 0:
        ...
        for small_cluster in small_clusters:
            cells_to_reassign = adata.obs['leiden'] == small_cluster
